...
pub struct RunOutcome {
    ...
    ///
    /// Task 014 turns this into `runs.resume_after`. This module deliberately
...
/// — but driving that loop is task 014's job, named out of scope by task 008's
/// own task file ("Retry and resume behaviour (014). Classification only."):
/// [`crate::runner::process::run_task`] mints a fresh id on every call it makes,
/// because it has no continuation prompt to send and no resume policy to apply
/// yet. See [`crate::runner::process::Invocation::resume`], which is `false`
...
pub async fn finish_run(ctx: &ServiceContext, run_id: &str, outcome: &RunOutcome) -> Result<Run> {
    ...
    apply_to_task(ctx, &run.task_id, outcome.exit_class).await?;
...
///
/// Every class transitions *out* of `running`, including the three the MVP does
/// not otherwise act on. A task left `running` with no process is a state
/// nothing can recover from and a badge that lies; `waiting_retry` is simply
/// where ADR-0011's table already puts a run that is going to be resumed — the
/// *acting* task 014 owns is scheduling the wait, not naming it.
async fn apply_to_task(ctx: &ServiceContext, task_id: &str, exit_class: ExitClass) -> Result<()> {
    if exit_class == ExitClass::Success {
        move_to_in_review(ctx, task_id).await?;
    }

    let run_state = match exit_class {
